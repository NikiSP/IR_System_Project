...
class Scorer:    
    ...
    def compute_scores_with_unigram_model(self, query, smoothing_method, document_lengths, alpha, lamda):
        """
    ...
    def compute_score_with_unigram_model(
        ...
        for term in query:
            ...
            if term in self.index:
                if document_id in self.index[term]:
                    tf_td= self.index[term][document_id]
                for doc in self.index[term]:
                    cf_t+= self.index[term][doc]
            
             
            if smoothing_method=='bayes':
                T= sum(document_lengths.values())
                score+= tf_q[term]*(np.log(((tf_td)+(alpha*(cf_t/T)))/(L_d+alpha)))
            elif smoothing_method== 'mixture':
                T= sum(document_lengths.values())
                score+= tf_q[term]*(np.log(((lamda*tf_td)/L_d)+(((1-lamda)*cf_t)/T)))
            else:
                T= len(self.index.keys())
                score+= tf_q[term]*(np.log((tf_td+(1/T))/(L_d+1)))                
