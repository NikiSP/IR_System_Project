...
class DocumentLengthsIndex:
    def __init__(self,path='C:/Users/FasleJadid/Desktop/IRProject/IR_System_Project/Logic/core/indexer/index/'):
        ...
        self.documents_index = Index_reader(path, index_name=Indexes.DOCUMENTS, index_type=None).index
    ...
    def get_documents_length(self, where):
        ...
        for doc in self.documents_index.values():
            if where in doc:
                if doc[where]:
                    doc_lengths[doc['id']]= 0
                    for section in doc[where]:
                        doc_lengths[doc['id']]+= len(section.split())
                else: doc_lengths[doc['id']]= 0
            else:
                doc_lengths[doc['id']]= 0
        return doc_lengths    
