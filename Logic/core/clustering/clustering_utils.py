...
class ClusteringUtils:
    ...
    def kmeans(self, emb_vecss: List, n_clusters: int, max_iter: int):
        
        emb_vecs= np.array(emb_vecss)
        centroids= emb_vecs[np.random.choice(len(emb_vecs), n_clusters, replace=False)]
        for i in range(max_iter):
            distances= np.linalg.norm(emb_vecs[:, np.newaxis]-centroids, axis=2)
            labels= np.argmin(distances, axis=1)
            
            new_centroids= []
            for j in range(n_clusters):
                centroid= emb_vecs[labels==j]
                if len(centroid)==0:
                    new_centroids.append(emb_vecs[random.randint(0, len(emb_vecs))])
                else:
                    new_centroids.append(centroid.mean(axis=0))
                
            new_centroids= np.array(new_centroids)
            # new_centroids= np.array([emb_vecs[labels==j].mean(axis=0) for j in range(n_clusters)])

            if np.all(centroids==new_centroids):
                break
            centroids= new_centroids
        
        return centroids.tolist(), labels.tolist(), distances
    
    
    def cluster_kmeans(self, emb_vecss: List, n_clusters: int, max_iter: int = 100) -> Tuple[List, List]:
        """
        ...
        centers, labels, distances= self.kmeans(emb_vecss, n_clusters, max_iter)
    ...
    def get_most_frequent_words(self, documents: List[str], top_n: int = 10) -> List[Tuple[str, int]]:
        ...
        counts, words= zip(*sorted(zip(word_counts.values(), word_counts.keys())))
    ...
    def cluster_kmeans_WCSS(self, emb_vecss: List, n_clusters: int) -> Tuple[List, List, float]:
        ...
        """
        centers, labels, distances= self.kmeans(emb_vecss, n_clusters, 100)
        return centers, labels, sum(np.min(distances, axis=1)**2)
    ...
    def cluster_hierarchical_single(self, emb_vecs: List, n_clusters) -> List:
        ...
        """
        single_model= AgglomerativeClustering(n_clusters= n_clusters, linkage='single')
        return single_model.fit_predict(emb_vecs)
    ...
    def cluster_hierarchical_complete(self, emb_vecs: List, n_clusters) -> List:
        ...
        return complete_model.fit_predict(emb_vecs)
    ...
    def cluster_hierarchical_average(self, emb_vecs: List, n_clusters) -> List:
        ...
        return average_model.fit_predict(emb_vecs)
    ...
    def cluster_hierarchical_ward(self, emb_vecs: List, n_clusters) -> List:
        ...
        return ward_model.fit_predict(emb_vecs)
    ...
    def wandb_plot_hierarchical_clustering_dendrogram(self, data, project_name, linkage_method, run_name):
        ...
        res_matrix= linkage(data, method=linkage_method)

        # Create linkage matrix for dendrogram
        plt.figure(figsize=(12, 6))
        dendrogram(res_matrix)
        plt.xlabel("Index")
        plt.ylabel("Dist")
        plt.title(f'Hierarchical Clustering Dendrogram ({linkage_method} linkage method)')
        
        fig= plt.gcf()
        wandb.log({f"{run_name}": wandb.Image(fig)})
        plt.close()
        # wandb.finish()
    ...
    def plot_kmeans_cluster_scores(self, embeddings: List, true_labels: List, k_values: List[int], project_name=None,
        ...
        for k in k_values:
            ...
            # and visualize it.
            # print(k)
            centers, cluster_labels= self.cluster_kmeans(embeddings, k)
            
            silhouette_scores.append(clustering_metrics.silhouette_score(embeddings, cluster_labels))
            purity_scores.append(clustering_metrics.purity_score(true_labels, cluster_labels))
        ...
        # Plotting the scores
        plt.figure(figsize=(12, 6))
        plt.plot(k_values, silhouette_scores, label='Silhouette Scores', marker='o')
        plt.plot(k_values, purity_scores, label='Purity Scores', marker='o')
    ...
    def visualize_elbow_method_wcss(self, embeddings: List, k_values: List[int], project_name: str, run_name: str):
        ...
        wcss_values = []
        for k in k_values:
            centers, indexes, wcss = self.cluster_kmeans_WCSS(embeddings, k)
