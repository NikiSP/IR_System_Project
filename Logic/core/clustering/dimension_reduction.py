...
class DimensionReduction:
    ...
    def wandb_plot_2d_tsne(self, data, project_name, run_name):
        ...
        res= self.convert_to_2d_tsne(data)

        # Plot the t-SNE embeddings
        plt.figure(figsize=(12, 6))
        plt.scatter(res[:, 0], res[:, 1], s= 5)
        plt.title("TSNE 2D Embeddings")
        ...
        fig= plt.gcf()
        wandb.log({"TSNE 2D Embeddings": wandb.Image(fig)})
    ...
    def wandb_plot_explained_variance_by_components(self, data, project_name, run_name):
        ...
        plt.ylabel("Explained Cum_Var Ratio")
        # plt.show()
        

        # Log the plot to wandb
        fig= plt.gcf()
        wandb.log({"Explained Variance by Components": wandb.Image(fig)})
